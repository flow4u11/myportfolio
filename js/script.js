...
function initScrollReveal() {
    ...
    revealElements.forEach(container => {
        ...
        gsap.fromTo(container,
            { transformOrigin: '0% 50%', rotate: 3, opacity: 0.5 },
            { ease: 'none', rotate: 0, opacity: 1, scrollTrigger: { trigger: container, start: 'top bottom', end: 'center center', scrub: true } }
        );

        gsap.fromTo(wordSpans,
            { opacity: 0.1, filter: `blur(4px)` },
            { ease: 'none', opacity: 1, filter: 'blur(0px)', stagger: 0.05, scrollTrigger: { trigger: container, start: 'top bottom-=10%', end: 'bottom center', scrub: true } }
        );
    });
}
